...
- `lingerMaxMs` (default 5000) lets an already-sung `Lyrics` event keep showing on its row after it would otherwise end, filling the gap up to when that row's next occupant needs it (its own `Preview` start, or its own `Lyrics` start if no preview shows), capped at `lingerMaxMs`. Where a real, song-wide quiet gap overlaps that row's idle window, the lingering ceiling is additionally clamped to the start of that quiet gap — not blocked outright — since a row's idle window can be much wider than any actual pause within it (other rows may have kept the screen busy for most of that window, with only its tail genuinely overlapping silence); a row simply being unused for a while — e.g. as a side effect of the rotation reset below skipping it for a cycle — is not by itself treated as a pause when other rows keep the screen continuously busy throughout. A quiet gap is measured against each occurrence's actual first-sung-word timing (`effectiveSungStartMs`), not its possibly pre-rolled/dot-prefixed display start, so a real pause hidden behind a count-in sweep (see `mainLinePreRollMs` below) is still detected correctly. A gap qualifies as quiet once it reaches `interlude.blankGapMs` (default 3000, clamped to never exceed `interlude.minGapMs`) with a non-`'none'` `interlude.strategy` configured — this is a lower bar than `interlude.minGapMs` itself, so a real pause too short to warrant a full `Interlude` event can still cap lingering short of it. `lingerMaxMs` is unconstrained by gap size when no `interlude` is configured. This is independent of `trailingLyricDurationMs`, which instead trims a lyric's own tail relative to its immediate next occurrence to make room for an interlude at all; `lingerMaxMs` decides how much of a same-row gap an older line keeps occupying once such a gap already exists. Note: `interlude`'s own placement (whether a full `Interlude` event appears at all) and `trailingLyricDurationMs`'s trimming decision both still key off display-anchored timing, not `effectiveSungStartMs`; only the lingering ceiling above uses real sung timing.
...
- A `text` interlude displays `♪ Instrumental ♪`; a `countdown` interlude emits one-second events labelled with whole seconds remaining; a `progress-bar` interlude draws a vertically-centered rounded-rectangle bar spanning the lyric safe area (`layout.marginLeft`/`marginRight`), as two drawing-mode (`\p1`) events sharing the gap's start/end: a static track (filled with `styles.interlude.secondaryColor`) and an animated fill on top (`primaryColor`, bordered with `outlineColor`) that reveals left-to-right over the gap's duration via a `\t`-animated `\clip` rectangle, rather than scaling the shape (which would distort its rounded corners). Both events force `\shad0`, since the style's own shadow (if any) would otherwise fall asymmetrically across the drawn shape.
